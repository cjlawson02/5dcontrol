...
1. Mobile connects to `ws://<ip>:8888/ws` (ports from mDNS TXT when browsed)
2. Commands: FlatBuffers `FOCUS` / `CAPTURE` / `QUERY_STATUS` / `QUERY_SETTINGS` / `QUERY_AVAILABLE_SETTINGS` / `SET_SETTING` (`FOCUS` may include normalized coords)
...
10. Server advertises `_5dcontrol._tcp` with TXT `http_port` / `ws_port`; iOS browses (dev client)
...
- `apps/server/discovery` — mDNS advertise `_5dcontrol._tcp` (SRV HTTP; TXT `http_port` / `ws_port`)
...
- `apps/mobile/components/CameraStream.tsx` — MJPEG WebView + native pinch/pan + tap-to-focus
- `apps/mobile/hooks/useMdnsBrowse.ts` — iOS Bonjour browse
- `apps/mobile/components/ViewfinderGlass.tsx` — glass HUD chrome
- `apps/mobile/app/(tabs)/index.tsx` — viewfinder + gallery-button last-capture thumb + exposure + tap-to-focus
...
**Discovery / tap-to-focus note:** Demo path is the acceptance path for M3. iOS Bonjour browse needs a native dev client (not Expo Go) plus Local Network permission. Tap places a reticle and sends `FOCUS` with 0–1 coords; the mock logs the point. Live 5D III AF-point selection is not verified.
...
- Prefer demo mode for UI work (covers capture → `IMAGE_READY` → gallery, the exposure pill, tap-to-focus, and mDNS advertise)
